        ...
        pingDevice(id, event) {
            ...
            .then(data => {
                ...
                if (data.is_online) {
                    ...
                    statusIndicatorSpan.parentNode.className = 'inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-rose-50 dark:bg-rose-950/30 text-rose-700 dark:text-rose-400 border border-rose-250 dark:border-rose-900/30';
                }
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <form action="{{ route('zkteco-devices.destroy', $device->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus perangkat absensi ini?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="p-1.5 hover:bg-rose-50 dark:hover:bg-rose-950/20 rounded-lg text-rose-600 dark:text-rose-400 hover:text-rose-750 transition-colors cursor-pointer" title="Hapus Mesin">
