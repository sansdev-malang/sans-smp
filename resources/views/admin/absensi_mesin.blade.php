                ...
                <table class="w-full text-xs border-collapse">
                    <thead>
                        <tr class="border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/30">
                            ...
                            <th class="px-6 py-4 text-center font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider w-48">Aksi</th>
                        </tr>
                    </thead>
        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <form id="add-form" onsubmit="submitForm(event)" class="p-6 space-y-4">
                ...
                <div class="space-y-1">
                    ...
                    <input type="text" required id="form-nama" placeholder="Contoh: Mesin Utama Lt. 1" class="w-full h-9 px-3 text-xs bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-slate-100 dark:focus:ring-slate-800">
                </div>

                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-1">
                        <label class="text-xs font-semibold text-slate-500 dark:text-slate-400">IP Address</label>
                        <input type="text" required id="form-ip" placeholder="192.168.1.201" class="w-full h-9 px-3 text-xs bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-slate-100 dark:focus:ring-slate-800">
                    </div>
                    <div class="space-y-1">
                        <label class="text-xs font-semibold text-slate-500 dark:text-slate-400">Port Comm</label>
                        <input type="number" required id="form-port" value="4370" class="w-full h-9 px-3 text-xs bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-slate-100 dark:focus:ring-slate-800">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="space-y-1">
                        ...
                        <input type="text" required id="form-lokasi" placeholder="Pintu Gerbang Utama" class="w-full h-9 px-3 text-xs bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-slate-100 dark:focus:ring-slate-800">
                    </div>
                </div>
        ...
        function renderTable() {
            ...
            dataMesin.forEach((item, index) => {
                ...
                if (item.status === 'Online') {
                    ...
                    statusBadge = `
                        <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-900/30">
                ...
                row.innerHTML = `
                    ...
                    <td class="px-6 py-4 text-slate-600 dark:text-slate-405 font-mono">${item.port}</td>
                    ...
                    <td class="px-6 py-4 text-center">
                        <div class="flex items-center justify-center gap-2">
                            ...
                            <button onclick="deleteMachine(${item.id})" class="p-1.5 hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded-lg text-rose-600 dark:text-rose-400 hover:text-rose-700 transition-colors cursor-pointer" title="Hapus Mesin">
